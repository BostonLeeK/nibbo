...
export async function GET(req: NextRequest) {
  ...
  const until = untilRaw && YMD.test(untilRaw) ? untilRaw : kyivCalendarYmd();
  const startYmd = kyivCalendarYmdMinusDays(until, days);
  const { start, end } = kyivRangeUtcFromCalendarYmd(startYmd, until);
