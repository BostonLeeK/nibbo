...
export default function Sidebar({ user: u }: SidebarProps) {
  ...
  return (
    <>
      <aside className="md:hidden bg-white/85 backdrop-blur-md border-b border-warm-100 px-3 pt-3 pb-2 shadow-sm">
            ...
            return (
              <Link key={item.href} href={item.href} className="shrink-0">
                <motion.div
                  whileTap={{ scale: 0.97 }}
                  className={cn(
                    "flex items-center gap-2 px-3 py-2 rounded-xl font-medium text-xs transition-all",
                    isActive
                      ? "bg-gradient-to-r from-rose-50 to-rose-100/70 text-rose-700 shadow-sm"
                      : "text-warm-600 bg-white hover:bg-warm-50"
                  )}
                >
